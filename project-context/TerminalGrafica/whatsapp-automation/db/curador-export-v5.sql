-- =============================================================================
-- EXPORT DEL CATÁLOGO A JSON — v5 (modelo producto-bot + trabajos con VARIANTES) · GREENFIELD
-- =============================================================================
...
--
-- Diferencia con v4: los TRABAJOS ya no se agrupan por parte (cartesiano libre). Ahora
-- cada trabajo tiene sus VARIANTES CERRADAS propias (bot.job_variant), y cada una enumera
-- sus componentes concretos (bot.job_variant_material → bot.variant que van juntas). El
-- precio de cada variante-de-trabajo = Σ de sus componentes (lo suma TS en chunkTrabajo).
-- El bloque 'productos' es idéntico a v4.
--
-- Uso (Martin): correr en el SQL editor del Supabase de TESTING → guardar como
--   db/export-actualizado-catalogo-v5.json → re-ingestar (pnpm rag:ingest).
-- SOLO SELECT. schema_version = 5.
--
  ...
  'productos', (
    ...
    from (
      select json_build_object(
        ...
        -- OJO: este json_build_object está DUPLICADO en 'trabajos'.variantes[].componentes (abajo) →
        -- cambio de campos = tocar los DOS bloques.
        'items', (
  ...
  'trabajos', (
    ...
    from (
      select json_build_object(
        ...
        -- variantes-de-trabajo: una por bot.job_variant no oculta, con sus componentes concretos.
        'variantes', (
          select coalesce(json_agg(vt order by (vt->>'ref_pos')::int, vt->>'nombre_bot'), '[]'::json)
          from (
            select json_build_object(
              'nombre_bot', jv.bot_name,
              'ref_pos',    jv.position,
              'componentes', (
                select coalesce(json_agg(json_build_object(
                    'variante_id',        pr.variant_id,
                ...
                join pricing pr on pr.variant_id = bv.variant_id
                where jvm.job_variant_id = jv.id and not bv.hidden
              )
            ) as vt
            from bot.job_variant jv
            where jv.job_id = bj.id and not jv.hidden
              -- no emitir una variante-de-trabajo sin componentes exportables
              and exists (
                select 1
                from bot.job_variant_material jvm
                join bot.variant bv on bv.id = jvm.bot_variant_id
      ...
      from bot.job bj
      where not bj.hidden
        -- no emitir un trabajo sin ninguna variante-de-trabajo exportable (chunk degenerado)
        and exists (
          select 1
          from bot.job_variant jv
          where jv.job_id = bj.id and not jv.hidden
