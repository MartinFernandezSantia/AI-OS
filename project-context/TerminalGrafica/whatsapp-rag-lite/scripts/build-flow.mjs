  ...
  nodes: [
    ...
    },
    {
      parameters: {
        ...
        // returnIntermediateSteps: expone las llamadas LLM del tool-call en intermediateSteps[] (con
        // usage + costo USD de OpenRouter) para telemetría de tokens. Aditivo: no cambia `output`.
...
const prepararEnvio = {
  ...
  parameters: {
    jsCode: [
      ...
      "// TOKENS (uso_llm): con returnIntermediateSteps=true el Agente expone sus llamadas LLM del",
      "// tool-call en intermediateSteps[].action.messageLog[].kwargs.response_metadata.usage, con el",
      "// costo USD real de OpenRouter. Referenciable acá porque es el output del propio Agente (nodo",
      "// main, NO un sub-nodo). COBERTURA PARCIAL (parcial:true): cubre la(s) ronda(s) con tool-call",
      "// del Agente (la cara: prompt + catálogo); NO la generación final ni Verificador/Corrector/",
      "// Guardrails → es un PISO del gasto, no el total.",
      ...
      "  }",
      "} catch (e) {}",
      "const uso_llm = { llamadas, tokens_in: tin, tokens_out: tout, costo_usd: usd, modelo, nodos: { agente: { in: tin, out: tout, usd } }, parcial: true };",
      "return [{ json: {",
  ...
  parameters: {
    ...
    columns: {
      ...
      value: {
        ...
        // uso_llm (tokens + costo USD): lo arma Preparar Envío desde intermediateSteps del Agente
        // (returnIntermediateSteps). parcial:true = piso (solo la ronda tool-call del Agente). Acá se serializa.
