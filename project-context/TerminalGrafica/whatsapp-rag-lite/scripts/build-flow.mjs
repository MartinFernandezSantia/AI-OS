...
const prepararEnvio = {
  ...
  parameters: {
    jsCode: [
      ...
      "// TOKENS (uso_llm): suma el tokenUsage de TODAS las corridas de cada nodo de modelo. El Agente",
      "// corre ≥2 veces (tool-call) → iteramos runIndex hasta que .all(0,r) se queda sin runs. Va en un",
      "// Code node (no en la expresión de Postgres) porque $(variable) sólo resuelve acá. tokenUsage =",
      "// {promptTokens, completionTokens, totalTokens} en el output del sub-nodo de modelo.",
      "const sumar = (nombre) => {",
      "  let inTok = 0, outTok = 0, llamadas = 0;",
      "  for (let r = 0; r < 25; r++) {",
      "    let items;",
      "    try { items = $(nombre).all(0, r); } catch (e) { break; }",
      "    if (!items || !items.length) break;",
      "    for (const it of items) {",
      "      const u = (it && it.json && it.json.tokenUsage) || {};",
      "      const pi = Number(u.promptTokens || 0), co = Number(u.completionTokens || 0);",
      "      if (pi || co) { inTok += pi; outTok += co; llamadas++; }",
      "    }",
      "  }",
      "  return { inTok, outTok, llamadas };",
      "};",
      "const modelos = { agente: 'Modelo', verificador: 'Modelo · Verificador', corrector: 'Modelo · Corrector', guardrails: 'Modelo · Guardrails' };",
      "const nodos = {}; let tin = 0, tout = 0, nLlam = 0;",
      "for (const clave in modelos) {",
      "  const s = sumar(modelos[clave]);",
      "  if (s.inTok || s.outTok) { nodos[clave] = { in: s.inTok, out: s.outTok }; tin += s.inTok; tout += s.outTok; nLlam += s.llamadas; }",
      "}",
      "const uso_llm = { llamadas: nLlam, tokens_in: tin, tokens_out: tout, nodos };",
      "return [{ json: {",
  ...
  parameters: {
    ...
    columns: {
      ...
      value: {
        ...
        // tokens por turno: los ensambla Preparar Envío (Code node, itera runIndex por modelo). Acá
        // sólo se serializa. Ver la nota en Preparar Envío / db/observabilidad-rag-2026-08-11.sql.
