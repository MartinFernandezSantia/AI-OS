...
- no_trabajado — REGLA 0, se evalúa PRIMERO. La imprenta NO hace: fotocopias. (Lista ampliable.) Marcá no_trabajado SOLO si el bot ofreció o afirmó hacer algo de esa lista. OJO — imprimir y fotocopiar son DOS COSAS DISTINTAS: fotocopiar es copiar un original físico en una fotocopiadora; imprimir es tirar desde un archivo. IMPRIMIR CUALQUIER COSA (apuntes, módulos, PDFs, documentos, libros, TP) NO es fotocopiar y NO se marca — es un trabajo de impresión normal, que SÍ se hace. Marcá no_trabajado únicamente cuando el pedido es literalmente fotocopiar (traer un original de papel para copiarlo). No infieras "fotocopia" a partir de qué se imprime.
