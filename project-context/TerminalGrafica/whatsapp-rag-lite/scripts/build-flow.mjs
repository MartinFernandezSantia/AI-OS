...
const prepararEnvio = {
  ...
  parameters: {
    jsCode: [
      ...
      "// NOTA: los tokens (uso_llm) NO se capturan acá. Los sub-nodos ai_languageModel de n8n no son",
      "// referenciables desde el flujo principal ($('Modelo') no resuelve) → se enriquecen aparte, en",
      "// batch, por execution_id contra la API de ejecuciones de n8n. Ver db/observabilidad-rag-*.sql.",
      "return [{ json: {",
  ...
  parameters: {
    ...
    columns: {
      ...
      value: {
        ...
        // uso_llm (tokens) NO se escribe en el hot-path: los sub-nodos de modelo no se leen desde el
        // flujo principal. Se enriquece en batch por execution_id (ya logueado). Ver observabilidad-rag-*.sql.
      },
      matchingColumns: [],
      schema: [],
    },
