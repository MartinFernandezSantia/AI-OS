...
const CODE_RESPONDER = [
  ...
  "//",
  "// Se detecta por el mail EN EL MENSAJE DEL MODELO, no por palabras clave del cliente: el",
  "// prompt manda a mail exactamente los casos de avanzar/archivos/estado, así que la",
  "// presencia del mail ES la señal de que el turno cierra hacia allá.",
  "//",
  "// NO aplica cuando el turno derivó por CONSULTA o por falla técnica: esos mensajes ya",
  "// dicen que no hay precio, y avisar que 'el precio no es final' cuando no se dio ninguno",
  "// confunde en vez de cubrir.",
  "const MAIL = 'terminalgrafica@gmail.com';",
  "if (via === 'normal' && output.includes(MAIL) && !output.includes(AVISO_PRECIO)) {",
  "  output += '\\n\\n' + AVISO_PRECIO;",
