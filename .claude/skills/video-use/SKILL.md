...
- **AIOS-local note (this machine):** deps live in this dir's uv venv, *not* system Python (Ubuntu's `python3` is 3.14 with no pip). Invoke every helper through the venv — `uv run python helpers/<name>.py ...` (run from this skill dir or pass `--project /home/martin/AI-OS/.claude/skills/video-use`), or call `/home/martin/AI-OS/.claude/skills/video-use/.venv/bin/python` directly. A bare `python helpers/...` will fail with `ModuleNotFoundError`. `ffmpeg`/`ffprobe` are static builds on `~/.local/bin` (already on PATH). See `/home/martin/AI-OS/SETUP.md`.
