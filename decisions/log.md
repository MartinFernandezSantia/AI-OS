...
El detalle que hace que funcione: el `repeatNote` del prompt principal es literalmente lo que ordena el silencio (*"si tu respuesta no agregaría nada nuevo... respondé con action noop"*). Reinyectar sin tocarlo era rescatar en el papel — el modelo recibía la misma orden, se callaba de nuevo y se gastaba la única vuelta que da la cota. En la 2ª vuelta ese mensaje se reemplaza por: no uses noop + esto es lo que falta resolver + si es precio usá `action precio`, y si el dato no existe decilo en vez de callarte. El `pendiente` se sanea antes de entrar al prompt (una línea, 300 chars, montos borrados: ningún LLM tipea plata, sin excepciones). Sólo se verifica el silencio que emitió el LLM (`noopOrigen` `llm` o `reply-vacio`) — el anti-loop determinístico de 2 repeticiones no se discute, es correcto por construcción y no paga la llamada. La cota anti-ciclo es el flag `reintentoSilencio`: en la 2ª vuelta el silencio se respeta sin verificar. Toda degradación (timeout, JSON ilegible, veredicto desconocido) cae a `callar`, o sea al comportamiento actual.
...
## 2026-07-29 — La oferta por cantidad se ofrece, aunque el cliente pida menos

**Decision:** cuando un producto tiene `min_unidades` y el cliente pide menos, el bot dice las DOS cosas: el precio de lo que pidió y la oferta por cantidad. El monto de la oferta viaja pegado a su condición ("$15.000 por unidad llevando 6 o más") dentro del campo que el compositor copia literal, no como aclaración aparte. Bajo el mínimo NO se calcula total con el precio de la oferta. El verificador tiene prohibido borrar una oferta al corregir, y un guard determinístico lo verifica: si la corrección perdió el monto de la oferta, se descarta la corrección y sale el borrador original.

**Why:** en la ronda real de WhatsApp el cliente dijo que era de una inmobiliaria y pidió 3 carteles. El bot cotizó bien el suelto ($19.500) y nunca mencionó la promo ($15.000 llevando 6). Fallaron dos capas a la vez: `Calcular Montos` hacía `continue` y descartaba la promo por no llegar al mínimo, y el verificador borró de un mensaje que sí la traía "la mención a la promoción de 6 unidades" por considerarla información innecesaria — con criterio correcto para lo que sabía, porque su prompt lo autorizaba a sacar relleno y nadie le había dicho que una oferta no lo es. El razonamiento viejo confundía dos cosas: cotizar 3 a $15.000 sería mentir (1,3x por debajo de los $58.500 reales) y eso sigue prohibido, pero *nombrar* la oferta con su mínimo no es mentir, es la razón de existir del producto. El mínimo pasa de caveat a estar dentro del `texto` del monto porque un caveat es una línea suelta que se puede resumir, mover o perder; ahí adentro es intocable y el número nunca viaja sin la condición que lo habilita. El guard compara MONTOS y no la redacción: comparar la frase rompería la libertad de reformular que se le dio al verificador a propósito. El prompt solo no alcanzaba: el verificador no falló por no saber la regla, falló juzgando relevancia, y es el último eslabón (nadie lo audita después).
...
## 2026-08-31 — La cantidad que declara el bot va SIN convertir a la unidad de cobro

**Decision:** el campo `cantidad` de la salida estructurada del cotizador es siempre lo que pidió el cliente en sus palabras, sin ninguna conversión: 100 stickers son 100 (no los 6 pliegos que ocupan), 1000 tarjetas son 1000 (no 1 paquete), una lona de 3x1 es 1 (no 3 m2), 2,45 metros de planos son 2.45. Toda conversión a la unidad de cobro —dividir por el rinde, dividir por el paquete, multiplicar por la superficie— la hace el nodo Code, nunca el modelo. La regla no admite excepciones y así quedó escrita en las tres capas que le hablan al modelo: el schema de salida, el system prompt y el texto de los chunks.

**Why:** la redacción anterior decía "cantidad EN LA UNIDAD DE COBRO que declara el catálogo", y el chunk de stickers dice en mayúsculas "Precio según CANTIDAD DE PLIEGOS A3 (no de piezas)". El modelo obedeció literal y se midieron dos cobros mal en vivo: 100 stickers de 7x7 declarados como 6 pliegos → el auditor volvió a dividir (6/18 = 1 pliego) y cobró $4.000 en vez de $16.800; una lona de 3x1 declarada como 3 m2 → el auditor volvió a multiplicar por la superficie (3 m2 x 3) y cobró $198.000 en vez de $66.000, el triple. Es la peor clase de falla del sistema: la aritmética cierra perfecto (1 pliego x $2.800 es correcto), así que el auditor no tiene contra qué comparar la premisa y la ejecución sale en verde sin hallazgos. El caso del metro lineal, que había motivado la redacción vieja, no era una excepción a "declarar lo que dijo el cliente" sino un ejemplo de ella: el cliente DICE "2,45 metros". Cuando una descripción de campo necesita un "pero si...", suele estar mal la regla y no el ejemplo.
