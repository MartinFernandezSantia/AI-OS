...
**Project & client registry — READ BEFORE assuming infra.** I run many projects at once (personal / client / demo; confirmed / unconfirmed; in-dev / stand-by). Single source of truth: `context/projects.md` (the projects + their infra owner) and `context/clients.md` (who each client is). **Whenever I mention a project, open `context/projects.md` first and check the "Infra y dueño" column before suggesting any VPS, server, account, domain, or budget.** Never share infra between projects, and never treat a client's unconfirmed resource (e.g. the TG Chatwoot Hostinger VPS) as available. I have NO production VPS of my own — `chatwoot.silvercoastwebagency.com` is a local dev VM, not prod. Note: TG and SantiaPropiedades projects currently sit in my personal Vercel/Supabase accounts (a known mistake to fix). Naming rule: refer to SantiaPropiedades by the business name, never as "my dad's business". When unsure of an owner/status, ask.
