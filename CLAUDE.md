...
**Project & client registry — READ BEFORE assuming infra.** I run many projects at once (personal / client / demo; confirmed / unconfirmed; in-dev / stand-by). Single source of truth: `context/projects.md` (the projects + their infra owner) and `context/clients.md` (who each client is). **Whenever I mention a project, open `context/projects.md` first and check the "Infra y dueño" column before suggesting any VPS, server, account, domain, or budget.** Never share infra between projects, and never treat a client's unconfirmed or not-yet-contracted resource as available (e.g. the TG bot's Hostinger KVM 4: confirmed by the jul-2026 sale but not contracted yet — check the registry for current state). I have NO production VPS of my own — `chatwoot.silvercoastwebagency.com` is a local dev VM, not prod. Note: TG and SantiaPropiedades projects currently sit in my personal Vercel/Supabase accounts (a known mistake to fix). Naming rule: refer to SantiaPropiedades by the business name, never as "my dad's business". When unsure of an owner/status, ask.
...
- If your fix doesn't work after two attempts, stop guessing — tell me exactly what you're uncertain about and what info or test you need from me before trying again.


## Git & Commit Discipline 
- Never commit directly to main in projects/ repos — always use a dedicated branch. This rule applies ONLY to projects/ repos, NOT the AIOS repo.
