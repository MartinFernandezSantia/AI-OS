...
> **AIOS note.** This repo denies `npm`/`npx` — run the HyperFrames CLI as **`pnpm dlx hyperframes …`** everywhere a reference shows `npx hyperframes …`. `ffmpeg`/`ffprobe` are already installed. The `MOTION_PHILOSOPHY.md` this skill assumes at the workspace root lives at **`references/MOTION_PHILOSOPHY.md`** (repo root) — read that when a Gate says to. For Gate 3 (style intake), the way to match a brand as tightly as the AIS examples is to build the project's `DESIGN.md` + `brand-tokens.css` first — follow **`references/hyperframes-brand-capture.md`**.
