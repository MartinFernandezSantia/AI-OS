...
> **AIOS note.** This repo denies `npm`/`npx` — run the HyperFrames CLI as **`pnpm dlx hyperframes …`** everywhere a reference shows `npx hyperframes …` (e.g. `pnpm dlx hyperframes render …`). `ffmpeg`/`ffprobe` are already installed. The aesthetic baseline lives at `references/MOTION_PHILOSOPHY.md` (repo root). To match a brand as tightly as the AIS examples do, follow `references/hyperframes-brand-capture.md` and build the project's `DESIGN.md` + `brand-tokens.css` first. This skill was distilled from Nate Herk's "May Shorts 19"; a source-only copy is bundled at `references/example-may-shorts-19/`.
...
- `references/example-may-shorts-19/` — the canonical short-form example (18.84s, 1080x1920, 7 scenes, face-mode choreography, karaoke captions with shift(), ambient bg + seam treatment), bundled **source-only** with this skill. Read `index.html` and any `compositions/scene*.html` before authoring a new short. Media (mp4/wav) and brand assets were stripped — it's a read reference, not a runnable project. The `--ais-*` CSS vars it references map to the brand-token pattern in `references/hyperframes-brand-capture.md`.
- `may-shorts-18` (the secondary reference with the 4 lessons baked in: BOTTOM scale=0.75, stamp-after-reveal timing, scene-1→2 fluid fade, scene-4 data-dashboard grid) is **not** bundled — the four lessons are captured in the "Lessons from may-shorts-18" section above, so you don't need the project files.
...
The original repo referenced Nate's private `feedback_*.md` / `project_ais_brand_specs.md` memory files for the rationale behind the 10 rules, the contrast/glow techniques, the 6-layer control-room background, and the verification gate. Those are **not** bundled — their substance is already inlined in this SKILL.md (the 10 rules, ambient-background stack, and verification gate sections above). For the broader aesthetic, read `references/MOTION_PHILOSOPHY.md`; to capture a specific brand's hex codes, fonts, and logo glow, build a `DESIGN.md` + `brand-tokens.css` per `references/hyperframes-brand-capture.md`.
