...
> **AIOS note.** This repo denies `npm`/`npx` — run the HyperFrames CLI as **`pnpm dlx hyperframes …`** everywhere a reference shows `npx hyperframes …`. `ffmpeg`/`ffprobe` are already installed. Aesthetic baseline: `references/MOTION_PHILOSOPHY.md` (repo root). To carry the captured site's brand through the video as tightly as the AIS examples, codify it into a `DESIGN.md` + `brand-tokens.css` per `references/hyperframes-brand-capture.md`.
